...
while not 0 < selk <= len(kernels):
    try:
        ...
        if defaultk == 0:
            print("There are no newer kernels available online")
            sys.exit(0)
...
sel4 = -1
while True:
    sel4 = input("Would you like to download kernel modules [Y/n]: ")
    if sel4 == "":
        selkm = True
        break
...
myCmd = 'dpkg -l | grep -i linux-image'
myCmdOutput = os.popen(myCmd).read()
print(myCmdOutput)
